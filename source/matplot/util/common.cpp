...
namespace matplot {
    ...
    bool is_true(const std::string &str) {
        return iequals(str, "on") || iequals(str, "true") || iequals(str, "yes");
    }

    bool is_false(const std::string& str) {
        return iequals(str, "off") || iequals(str, "false") || iequals(str, "no");
    }
    ...
    std::vector<double> linspace(double d1, double d2, size_t n) {
        ...
        for (size_t i = 0; i < x.size(); ++i) {
            x[i] = d1 + static_cast<double>(i) * (d2 - d1)/static_cast<double>(n - 1);
        }
        return x;
    }
    ...
    std::vector<double> logspace(double d1, double d2, size_t n) {
        return transform(linspace(d1,d2,n), [](double x) {return pow(10,x);});
    }
    ...
    std::vector<double> iota(double d1, double d2) {
        return iota(d1,1.,d2);
    }

    std::vector<double> transform(const std::vector<double>& x, std::function<double(double)> fn) {
    ...
    std::vector<double> transform(const std::vector<double>& x, const std::vector<double>& y, std::function<double(double,double)> fn) {
...
#undef _TEMP_MACRO_
    ...
    std::vector<double> rande(size_t n, double lambda, double multiplier) {
        std::vector<double> x(n);
        std::generate(x.begin(), x.end(), [&]{return rande(lambda,multiplier);});
        return x;
    }
    ...
    std::vector<double> rand(size_t n, double lower_bound, double upper_bound) {
        std::vector<double> x(n);
        std::generate(x.begin(), x.end(), [&]{return rand(lower_bound,upper_bound);});
        return x;
    }
    ...
    std::pair<vector_1d,vector_1d> elliptic2cart(vector_1d theta, vector_1d rho_w, vector_1d rho_h) {
        ...
        for (size_t i = 0; i < theta.size(); ++i) {
            std::tie(r.first[i], r.second[i]) = elliptic2cart(theta[i], rho_w[i], rho_h[i]);
        }
        return r;
    }
    ...
    std::pair<vector_1d,vector_1d> elliptic2cart(vector_1d theta, double rho_w, double rho_h) {
        ...
        r.second.resize(theta.size());
        for (size_t i = 0; i < theta.size(); ++i) {
            std::tie(r.first[i], r.second[i]) = elliptic2cart(theta[i], rho_w, rho_h);
        }
        return r;
    }
    ...
    std::pair<vector_1d,vector_1d> pol2cart(vector_1d theta, vector_1d rho) {
        return std::make_pair(
                transform(theta, rho, [](double theta, double rho) { return rho * cos(theta); }),
                transform(theta, rho, [](double theta, double rho) { return rho * sin(theta); })
        );
    }
    ...
    std::pair<vector_2d,vector_2d> gradient(const vector_2d& z, double spacing_x, double spacing_y) {
        ...
        for (size_t row = 0; row < n1; ++row) {
            ...
            for (size_t col = 1; col < n2 - 1; ++col) {
                dx[row][col] = ((z[row][col] - z[row][col-1]) + (z[row][col+1] - z[row][col])) / spacing_x / 2.;
            }
        }
        for (size_t col = 0; col < n2; ++col) {
            ...
            for (size_t row = 1; row < n1 - 1; ++row) {
                dy[row][col] = ((z[row][col] - z[row-1][col]) + (z[row+1][col] - z[row][col])) / spacing_y / 2.;
            }
        }
        return std::make_pair(dx,dy);
    }

    std::pair<vector_2d,vector_2d> gradient(const vector_2d& z, double spacing) {
    ...
    image_channels_t cimg2channels(const cimg_library::CImg<unsigned char>& cimg_image) {
        const size_t n_channels = cimg_image.spectrum();
        const size_t height = cimg_image.height();
        const size_t width = cimg_image.width();
        image_channels_t img(n_channels, image_channel_t(height, image_row_t(width, 0)));
        for (size_t channel = 0; channel < n_channels; ++channel) {
            for (size_t i = 0; i < cimg_image.height(); ++i) {
                for (size_t j = 0; j < cimg_image.width(); ++j) {
                    img[channel][i][j] = cimg_image.operator()(j,i,0,channel);
                }
            }
        }
        return img;
    ...
    cimg_library::CImg<unsigned char> channels2cimg(const image_channels_t& A) {
        cimg_library::CImg<unsigned char> cimg_image(A[0].size(),A[0][0].size(),1,A.size());
    ...
    image_channels_t imresize(const image_channels_t& A, size_t height, size_t width, image_interpolation m) {
    ...
    image_channels_t imresize(const image_channels_t& A, double scale, image_interpolation m) {
        if (A.empty()) {
    ...
    image_channels_t gray2rgb(const image_channel_t& A, const std::vector<std::vector<double>>& colormap) {
        image_channels_t img(3, image_channel_t(A.size(), image_row_t(A[0].size())));
        for (size_t i = 0; i < A.size(); ++i) {
            for (size_t j = 0; j < A[i].size(); ++j) {
                color_array c = colormap_interpolation(A[i][j],0,255,colormap);
    ...
    image_channels_t imvignette(const image_channels_t& A, double min_radius, double exponent) {
        auto image = A;
        if (image.empty()) {
            return A;
        }
        ...
        for (int i = 0; i < h; ++i) {
            for (int j = 0; j < w; ++j) {
                ...
                if (t1 + t2 > radius_sq) {
                    double norm_dist_from_r = ((t1 + t2) - radius_sq) / max_t_minus_radius;
    ...
    std::vector<std::string> tokenize(const std::string &text, std::string delimiters) {
        ...
        size_t pos = 0;
        while ((pos = text.find_first_not_of(delimiters, pos)) != std::string::npos) {
    ...
    std::pair<std::vector<std::string>,std::vector<size_t>> wordcount(const std::vector<std::string> &tokens, const std::vector<std::string> &black_list, const std::string &delimiters, size_t max_cloud_size) {
        // count the frequency of each token
        const bool bl_sorted = std::is_sorted(black_list.begin(), black_list.end());
        std::map<std::string, size_t> word_frequency;
        for (const auto &token : tokens) {
            const bool is_in_black_list = bl_sorted ?
                                          std::binary_search(black_list.begin(), black_list.end(), token) :
                                          std::find(black_list.begin(), black_list.end(), token) != black_list.end();
            if (!is_in_black_list) {
    ...
    std::pair<std::vector<std::string>,std::vector<size_t>> wordcount(const std::string &text, const std::vector<std::string> &black_list, const std::string &delimiters, size_t max_cloud_size) {
        auto tokens = tokenize(text);
        return wordcount(tokens, black_list, delimiters, max_cloud_size);
    }

    ticks_results calcticks(double limits_min, double limits_max, bool horizontal, double text_size, bool separateExp, bool log) {
        ...
        if (log) {
            std::invalid_argument("Not implemented yet. The library does not need that as it is.");
        }
        ...
        std::vector<double> roughInts = transform(testTickCounts, [&](double x) { return range/(x-1); });
        ...
        std::vector<double> decRoughInts = transform(roughInts, [](double x) { return floor(log10(x)); });
        std::vector<double> normRoughInts = transform(roughInts, decRoughInts, [](double x, double y) { return x/pow(10.,y); });
        ...
        std::vector<double> lLims = transform(niceInts, [&](double x) { return floor(limits_min/x)*x; });
        std::vector<double> uLims = transform(niceInts, [&](double x) { return ceil(limits_max/x)*x; });
        ...
        for (size_t i = 0; i < nTicks.size(); ++i) {
            nTicks[i] = floor(1 + (uLims[i] - lLims[i] + 10*minEps)/niceInts[i]);
        }
        ...
        std::vector<double> labelChars = transform(decInts, decMax, [&](double x, double y) {
        ...
        if (separateExp) {
            for (size_t i = 0; i < decInts.size(); ++i) {
                // Large numbers:
                isExp[i] = (decMax[i] > 6) || (decMax[i] == 6 && decInts[i] > 0);
                // Small numbers:
                isExp[i] = isExp[i] || (decMax[i] < -3) || (decMax[i] == -3 && decInts[i] < -5);
            }
        } else {
            for (size_t i = 0; i < decInts.size(); ++i) {
                // Large numbers:
        ...
        if (separateExp) {
            expChars = transform(decInts, decMax, [&](double x, double y) {
                return 2 + max(0., min(static_cast<double>(maxChars) - 3, y - x));
            });
        } else {
            expChars = transform(decInts, decMax, [&](double x, double y) {
                return 2 + max(0., min(max(0., static_cast<double>(maxChars) - 7), y - x)) + 4;
            });
        }
        ...
        if (horizontal) {
            labelSize = transform(labelChars, [&](double labelChars) { return text_size * labelChars; });
        } else {
            labelSize = transform(labelChars, [&](double labelChars) { return text_size; });
        }
        ...
        for (size_t i = 0; i < nMax.size(); ++i) {
            nMax[i] = floor((uLims[i]-lLims[i]+10*minEps)/labelSize[i]) + 1;
        }
        // Modify this value based on initMaxTicks
        std::vector<double> nMaxScore = transform(nMax, [&](double x) { return min(x,initMaxTicks + 0.25*(x-initMaxTicks)); });
        // Calculate a score based on the number of ticks relative to the maximum.
        std::vector<double> nTickScore = transform(nTicks, nMaxScore, [](double x, double y) { return 1-pow((x/y - 0.7),2); });
        // Severe penalty for more than nMax ticks
        std::vector<double> penalty = transform(nTicks, nMax, [](double x, double y) { return 4*x/y; });
        for (size_t i = 0; i < nTicks.size(); ++i) {
        ...
        std::vector<double> penaltyScale = transform(nTicks, nMax, [](double x, double y) { return x/y; });
        ...
        for (size_t i = 0; i < isInt.size(); ++i) {
            scores[i] = isInt[i] + 0.75*hitsEnds[i] + nTickScore[i] - penalty[i];
        }
        ...
        std::vector<double> overhang = {max(0.,limits_min+labelSize[bestIdx]/2.-lLim),
        ...
        if (isExp[bestIdx] && separateExp) {
            ...
            int n = max(1., min(max(0., static_cast<double>(maxChars) - 3), decMax_d - decInt));
            std::string fStr = "%." + num2str(n) + 'f';
            // Normalize the ticks to the correct scale and create the labels
            std::vector<double> normTicks = transform(ticks, [&](double x) { return x / pow(10, decMax_d); });
            ...
            // Determine maximum number of decimals and set formatting string
            int n = max(0., min(max(0., static_cast<double>(maxChars) - 7), decMax_d - decInt));
            ...
            // Determine maximum number of decimals and set formatting string
            int n = max(0., min(-decInt, max(0., static_cast<double>(maxChars) - 3) - decMax_d));
            std::string fStr = "%." + num2str(n) + "f";
